...
build_graph <- function( t.salary, unit )
{
  ...
  xmax <- ifelse(all(is.na(t.salary$q50)), 50000, round(max(t.salary$q50, na.rm = TRUE), -3) + 50000)
...
create_salaries_table <- function(t.salary){
  t.salary <-  
  d %>% 
...
top_5_highest_paid <- function(d) {
  d %>%
    mutate(salary = dollarize(salary)) %>%
    filter(Department.Description == i) %>%  # Filter for the given department
    arrange(desc(salary)) %>%  # Sort by salary in descending order
    select(Full.Name, title, gender, salary) %>%  # Select relevant columns
    head(5)  # Get the top 5 highest paid individuals
}
